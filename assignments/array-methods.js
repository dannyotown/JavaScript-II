...
runners.forEach(function(item){
    fullnamer = item.first_name + ' ' + item.last_name;
    fullName.push(fullnamer);
})
...
runners.map(function(item){
    upperNames = item.first_name.toUpperCase();
...
var getTotal = runners.reduce((sum,runDesc)=>{
    return sum + runDesc.donation;
},0);
